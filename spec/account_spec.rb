...
describe Account do
  ...
  it "adds a withdrawal transaction to the log" do
    subject.withdraw(5)
    expect(subject.account_log.transactions).to eq [{date: today, debit:5, balance:-5}]
  end
  it "adds a deposited transaction to the log" do
    subject.deposit(10)
    expect(subject.account_log.transactions).to eq [{date: today, credit:10, balance:10}]
  end
end
